...
\def\LOTItemSpacing{6 pt}                   % spacing between LOT/LOF items
\def\BibItemSpacing{0.35\baselineskip}      % spacing between bibliographic items in reference
...
\documentclass[\BaseFont]{article}          % document class (article w/ 12 pt)
...
\begin{center}
    ...
    {\TitleFont {\LaTeX\ template for dissertation proposal} \par}  
...
\vspace{0.5in}
\begin{center}
    Baltimore, Maryland \\          % location (according to Hopkins it's always same)
    Month YEAR                      % like May 2024

    \vspace{0.25 in}
    
    %% copyright statement is optional (but this will protect your ideas)
    \copyright\ Author name \\      % author name again
    All rights reserved 
\end{center}
...
\hypersetup{linkcolor=black}                  % local hyperref settings to make the TOC appear black (you can comment it if you would like default)
